...
#define SY_STR2_EQ(p, q)   ((p)[0] == (q)[0] && (p)[1] == (q)[1])
#define SY_STR3_EQ(p, q)   (STR2_EQ(p, q) && (p)[2] == (q)[2])
#define SY_STR4_EQ(p, q)   (STR2_EQ(p, q) && STR2_EQ(p + 2, q + 2))
#define SY_STR5_EQ(p, q)   (STR2_EQ(p, q) && STR3_EQ(p + 2, q + 2))
#define SY_STR6_EQ(p, q)   (STR3_EQ(p, q) && STR3_EQ(p + 3, q + 3))
#define SY_STR7_EQ(p, q)   (STR3_EQ(p, q) && STR4_EQ(p + 3, q + 3))

#define SY_PARSE_ERR_ON(cond, err) {   \
    if (cond)                       \
        return (err);               \
}

static sy_vector_t header_values;
static sy_vector_t header_value_params;

static int sy_parse_uri(sy_uri_t* r, char* p);
static int sy_parse_method(char* begin, char* end);
static void sy_split_header_value(sy_string_t* val, char split, sy_vector_t* vec);

void sy_parse_init(void) {
    sy_vector_init(&header_values, sizeof(sy_string_t), 0);
    sy_vector_init(&header_value_params, sizeof(sy_string_t), 0);
}

static inline int sy_parse_method(char* begin, char* end) {
    // End of method
    int len = end - begin;
    switch (len) {
    case 3:
        if (SY_STR3_EQ(begin, "GET"))
            return M_GET;
        else if(SY_STR3_EQ(begin, "PUT"))
            return M_PUT;
        else
            return ERR_INVALID_METHOD;
        break;
    case 4:
        if (SY_STR4_EQ(begin, "POST"))
            return M_POST;
        else if (SY_STR4_EQ(begin, "HEAD"))
            return M_HEAD;
        else
            return ERR_INVALID_METHOD;
        break;
    case 5:
        if (SY_STR5_EQ(begin, "TRACE"))
            return M_TRACE;
        else
            return ERR_INVALID_METHOD;
        break;
    case 6:
        if (SY_STR6_EQ(begin, "DELETE"))
            return M_DELETE;
        else
            return ERR_INVALID_METHOD;
        break;
...
int sy_parse_request_line(sy_request_t* r) {
    sy_buffer_t* b = &r->rb;
    int uri_err;
    char* p;
    for (p = b->begin; p < b->end; ++p) {
        ...
        case RL_S_METHOD:
            ...
            case 'A' ... 'Z':
                break;
            case ' ':
                r->method = sy_parse_method(r->request_line.data, p);
        ...
        case RL_S_SP_BEFORE_URI:
            ...
            default:
                ...
                if ((uri_err = sy_parse_uri(&r->uri, p)) != OK)
                    return uri_err;
                r->state = RL_S_URI;
                break;
            }
        ...
        case RL_S_HTTP_H:
            SY_PARSE_ERR_ON(ch != 'T', ERR_INVALID_REQUEST);
        ...
        case RL_S_HTTP_HTT:
            SY_PARSE_ERR_ON(ch != 'P', ERR_INVALID_REQUEST);
        ...
        case RL_S_HTTP_VERSION_MAJOR:
            ...
            case '0' ... '9':
                r->version.major =
                        r->version.major * 10 + ch - '0';
                SY_PARSE_ERR_ON(r->version.major > 999, ERR_INVALID_VERSION);
                break;
            case '.':
        ...
        case RL_S_HTTP_VERSION_MINOR:
            ...
            case '0' ... '9':
                r->version.minor =
                        r->version.minor * 10 + ch - '0';
                SY_PARSE_ERR_ON(r->version.minor > 999, ERR_INVALID_VERSION);
                break;
            case ' ':
 ...
 */
static int sy_parse_uri(sy_uri_t* uri, char* p) {
    char ch = *p;
    switch (uri->state) {
    case URI_S_BEGIN:
        switch (ch) {
...
void sy_parse_header_host(sy_request_t* r) {
    sy_string_t* host = &r->headers.host;
    char* semicolon = NULL;
    ...
    if (semicolon == NULL) {
        ...
        r->host = (sy_string_t){host->data, semicolon - host->data};
